...
import { OpenInAppCard } from "../../src/components/OpenInAppCard";
...
export default function GameShareLanding() {
  ...
  useEffect(() => {
    if (!GAMES_TAB_ENABLED || showAppPrompt) return;
    if (status === "signed-out" && linkToken) {
      router.replace("/sign-in");
    }
  }, [status, linkToken, router, showAppPrompt]);

  const resolveQuery = useQuery({
    queryKey: queryKeys.games.shareLink(linkToken ?? ""),
    queryFn: () => fetchGameShareLink(linkToken ?? "", authToken),
    enabled: GAMES_TAB_ENABLED && !!linkToken && status === "signed-in" && !showAppPrompt,
