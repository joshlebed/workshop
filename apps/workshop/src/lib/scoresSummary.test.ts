...
import { buildTodaysGameScoresSummary, summarizeGameScoreBody } from "./scoresSummary";
...
// Per-game distillation runs through `summarizeGameScoreBody` everywhere a
// score is shown (Games cards, the clipboard recap). The registry formatters
// themselves are unit-tested in `packages/shared/src/gameRegistry.test.ts`;
// these lock the adapter — detection → registry formatter → fallback → URL
// stripping → scoreValue → null.
describe("summarizeGameScoreBody — per-game formatting", () => {
  it("formats maptap by dropping the URL/date header", () => {
    const raw = "www.maptap.gg May 27\n100🎯 95🏆 94🏅 52😔 77😂\nFinal score: 770";
    expect(summarizeGameScoreBody(game("a", "maptap", "https://maptap.gg/"), entry("u", raw))).toBe(
      "100🎯 95🏆 94🏅 52😔 77😂\nFinal score: 770",
    );
  });
  ...
  it("formats anthropeum as one clean line: grid + bare score, header + brag dropped", () => {
    ...
    expect(
      summarizeGameScoreBody(game("a", "Anthropeum", "https://anthropeum.com"), entry("u", raw)),
    ).toBe("🟨🟨🟨🟨🟩🟦🟩🟥🟦🟩 62,090");
  });
  ...
  it("formats Globle as the grid line ending in `= N`", () => {
    ...
    expect(
      summarizeGameScoreBody(game("a", "Globle", "https://globle-game.com/game"), entry("u", raw)),
    ).toBe("⬜🟨⬜🟧🟩 = 5");
  });
  ...
  it("formats Satle as `<grid> <fraction>` from the `Satle #N N/6` header", () => {
    ...
    expect(summarizeGameScoreBody(game("a", "Satle", "https://satle.ca/"), entry("u", raw))).toBe(
      "🟥🟥🟥🟥🟥🟩 6/6",
    );
  });
  ...
  it("formats travle as `<grid> +N` from the `#travle #N +N` header", () => {
    ...
    expect(
      summarizeGameScoreBody(game("a", "travle", "https://travle.earth"), entry("u", raw)),
    ).toBe("🟧✅🟩🟧🟩✅✅ +2");
  });
  ...
  it("formats Daily Tens by transposing the 5×2 grid into two rows of five", () => {
    ...
    expect(
      summarizeGameScoreBody(game("a", "Daily Tens", "https://dailytens.com/"), entry("u", raw)),
    ).toBe(["🏆🏆🏆❌❌", "❌❌❌🏆❌"].join("\n"));
  });
  ...
  it("formats Tradle as a per-guess green-count sparkline with the `N/6` suffix", () => {
    ...
    expect(
      summarizeGameScoreBody(game("a", "Tradle", "https://tradle.net/"), entry("u", raw)),
    ).toBe("🟩 2·3·4·4·4·5 6/6");
  });
  ...
  it("formats GeoSports as `<emoji grid>\\n<score / max>`", () => {
    ...
    expect(
      summarizeGameScoreBody(game("a", "GeoSports", "https://www.geosports.app"), entry("u", raw)),
    ).toBe("🟡🟡🔴🟡🟢\n711 / 1,000");
  });
  ...
  it("formats NYT Mini as the `M:SS` solve time rendered as keycap-emoji digits", () => {
    ...
    expect(
      summarizeGameScoreBody(
        game("a", "NYT Mini", "https://www.nytimes.com/crosswords/game/mini"),
        entry("u", raw),
      ),
    ).toBe("0️⃣:1️⃣6️⃣");
  });
  ...
  it("falls back to a cleaned raw copy for games without a heuristic, preserving alignment", () => {
    ...
    expect(summarizeGameScoreBody(game("a", "Some Game", ""), entry("u", raw))).toBe(
      ["Some Game #123", "  ▓▓░░", "  ░░▓▓"].join("\n"),
    );
  });

  it("falls back to scoreValue when scoreRaw is null", () => {
    expect(summarizeGameScoreBody(game("a", "Wordle", ""), entry("u", null, 4))).toBe("4");
  });

  it("returns null when there's no usable signal", () => {
    expect(summarizeGameScoreBody(game("a", "Wordle", ""), entry("u", null, null))).toBeNull();
    expect(summarizeGameScoreBody(game("a", "Wordle", ""), entry("u", "   \n\n", null))).toBeNull();
  });
  ...
  it("returns null for a URL-only Daily Tens share instead of surfacing the URL ref id", () => {
    ...
    expect(
      summarizeGameScoreBody(
        game("a", "Daily Tens", "https://dailytens.com/"),
