 ...
 *
 * The backend caps the cover photo at ~1.5MB of base64. We aim well under
 * that by capping quality at 0.5 and letting the picker downscale via
 * `allowsEditing: true` (which forces a crop step on iOS that also
 * resamples to a reasonable size). If a future regression makes the
 * resulting payload too large, add expo-image-manipulator and resize
 * here to ~1024px max edge.
 */
export async function pickCoverPhoto(): Promise<PickedCover | null> {
