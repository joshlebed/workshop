...
export const onRequestGet = async (context: PagesContext): Promise<Response> => {
  ...
  // The API round-trip and the font load are independent — overlap them.
  const [preview, fonts] = await Promise.all([
    fetchGameSharePreview(token, context.env),
    loadOgFonts(context.env, context.request.url),
  ]);
  const html = buildGameShareOgImageHtml(preview, ogAssetsFor(context.request.url));
