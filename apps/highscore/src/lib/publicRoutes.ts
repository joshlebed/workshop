...
//
// `/support`, `/privacy` and `/terms` are the canonical URLs published to the App Store
...
//
// Keep the literals exactly `/support`, `/privacy` and `/terms`: the first
// two are registered in App Store Connect and the third is the EULA the
// sign-in screen links to (Guideline 1.2), so none can drift without a
// metadata update.
...
export const PUBLIC_ROUTES = [SUPPORT_ROUTE, PRIVACY_ROUTE, TERMS_ROUTE] as const;
