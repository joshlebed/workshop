...
export const tokens = {
  ...
  // Staccato spacing (see UX-EXPLORATION.md): mostly tight 2/4/8 steps with
  // rare large breaks, instead of a uniform 4pt ladder. Sections breathe; rows
  // do not.
  space: { hair: 2, xs: 4, sm: 8, md: 14, lg: 20, xl: 28, xxl: 40 },
  /** Timeline spine gutter — the fixed left rail every feed row hangs off. */
  gutter: 28,
  ...
  font: {
    pixel: PIXEL_FONT,
    size: { xxs: 11, xs: 12, sm: 13, md: 15, lg: 18, xl: 22, xxl: 28 },
    weight: {
  ...
  // Snappy and stepped, never bouncy. `frame` is one animation step: state
  // changes advance in 2 or 4 hard frames, structural moves ease out in
  // `fast`/`base`.
  motion: { frame: 30, fast: 120, base: 150, stagger: 24 },
} as const;
...
/**
 * Text-side twin of `glow` — same restraint rules. Used only on the wordmark
 * and on a celebration numeral (a cleared day, a new streak).
 */
