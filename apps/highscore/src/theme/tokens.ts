...
/**
 * Deck geometry. The whole app hangs off one asymmetric vertical rule: a
 * fixed `gutter`-wide marker column on the left (day markers, rank numerals,
 * the shelf key) and content in the remaining width. The rule is drawn as a
 * 2px bezel only where the gutter carries something.
 */
export const deck = {
  /** Marker column width — the app's single vertical rule sits at its edge.
   *  Sized so a four-character word fits on one line in Press Start 2P at its
   *  10px minimum (4 × 10px + letterSpacing, plus the column's padding). */
  gutter: 76,
  /** Cartridge strip (top) — glyph cells plus the active underline. */
  stripHeight: 48,
  /** Control panel (bottom) — three keys. */
  panelHeight: 56,
  /** Recap bar that appears above the panel once you've posted today. */
  recapHeight: 32,
  /** Shelf grid columns. */
  shelfColumns: 3,
  /** How many days deep a cartridge scrolls, today inclusive. */
  daysDeep: 7,
  /** One motion step. Stepped transitions travel `step` twice — see `stepped`. */
  step: 8,
} as const;

/**
 * The app's one transition: 2 hard frames over 120ms. Everything that isn't
 * direct manipulation (tab switch, shelf zoom, indicator jump) uses it, so
 * the whole UI moves like a machine advancing a frame rather than easing.
 */
export const STEP_DURATION_MS = 120;

/** Neon glow for text. `glow()` shadows a box; letters need a text shadow. */
