  ...
  it("echoes an allowed origin on preflight", async () => {
    ...
    const res = await app.request("/v1/lists", {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "X-Workshop-Session-Version, X-Workshop-Client",
      },
    });
    ...
    expect(res.headers.get("access-control-allow-headers")).toContain("X-Workshop-Client");
  });
