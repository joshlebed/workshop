...
- **Phase 4** — iOS share extension. **4a-1 done** — JS-only
  ...
  happy-path). **4a-2 done (this PR)** — `expo-share-intent` plugin
  injects the Share Extension target, App Group entitlement
  (`group.dev.josh.workshop`), and `workshop://` URL scheme; the Swift
  extension reads `extensionContext.inputItems` for URLs / web pages /
  text and opens the parent app via the registered scheme;
  `useShareIntent` in `app/_layout.tsx` forwards the payload to
  `/share?url=…` (which the existing redirect maps to
  `/share/pick-list?url=…`). `app.json` `version` bumped to `0.2.0` to
  satisfy the `runtimeVersion.policy: "appVersion"` rule (CLAUDE.md
  iOS deploy pipeline § "appVersion is safe when paired with a version
  bump"). Verification requires a real iPhone — TestFlight build
  auto-fires on merge via `testflight.yml` and `@expo/fingerprint`.
...
**4a-2 shipped early (out of the originally-planned Phase 5 ordering)** —
prioritized when human reported "Workshop doesn't show up in iPhone Share to
panel." The deferral rationale below is preserved for historical context; the
risks it cites still apply to the manual TestFlight smoke test step.
...
| **4a-2** | Native iOS share extension via `expo-share-intent` (`^6.1.0`) config plugin. Plugin injects the Share Extension target (bundle id `dev.josh.workshop.share-extension`, display name "Workshop Share"), App Group entitlement (`group.dev.josh.workshop`), `NSExtensionActivationRule` for URLs / web pages / text (no images for v1), and stock Swift `ShareViewController` that stashes the payload in App Group `UserDefaults` and opens `workshop://dataUrl=…`. `useShareIntent` hook in `app/_layout.tsx` reads the payload back (cold-launch via `expo-linking`'s initial URL + app-resume via `AppState`), routes signed-in users to `/share?url=…` which the existing `app/share/index.tsx` redirect chain (4a-1) carries to `/share/pick-list?url=…` → `/list/:id/add?prefillUrl=…`. `app.json` `version` bumped to `0.2.0` to satisfy `runtimeVersion.policy: "appVersion"`. `CFBundleURLSchemes` array gains `"workshop"` alongside the Google OAuth scheme (the plugin only auto-adds it when the array isn't already overridden). EAS native build auto-triggers via `@expo/fingerprint`. | App Group `group.dev.josh.workshop` — already _registered_ in the Apple portal (see CLAUDE.md "iOS capabilities are config-as-code"); EAS sync re-enables the App ID capability on the next build because the config plugin now declares it. EAS will also create the new `dev.josh.workshop.share-extension` App ID + matching provisioning profile automatically on the first build. | **Done (this PR)** |
